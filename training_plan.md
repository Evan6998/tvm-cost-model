...
- Decide the policy for MetaSchedule-driven training. Suggested:
	- Primary path: use `fit_measurements` with ranking pairs.
...
- Periodically (e.g., every N new results), create MeasurementRecord objects from this buffer and call `pipeline.fit_measurements(...)`.
	- Optional: add a pointwise regression path for pretraining or ablations:
...
- [x] Create a MetaSchedule-compatible cost model wrapper  
  GraphPyCostModel (PyCostModel-compatible) now lives in `src/tvm_cost_model/integration/metaschedule_adapter.py`, buffers measurements, and returns float64 numpy scores (higher is better).
- [x] Implement conversion from MetaSchedule objects to internal data  
  Added `runner_result_to_latency_ms`, `candidate_to_tir`, and `pack_measurements` in `src/tvm_cost_model/integration/utils.py`.
- [x] Plug pairwise training into update  
  GraphPyCostModel buffers MeasurementRecords and triggers `TrainingPipeline.fit_measurements`; it falls back to pointwise regression when no pairs are available.
- [x] Clarify / implement score transformation  
  Introduced `measurement_to_score` (negative log-latency, larger is better) and use it for pointwise updates from measurements.
- [x] Generalize TrainingPipeline._build_graph & TVMGraphBuilder  
  Builders accept str/IRModule/PrimFunc/Schedule inputs with type dispatch; pipeline fit/predict paths handle broader inputs.
- [x] Remove / deprecate the old MetaScheduleAdapter  
  MetaScheduleAdapter is now a deprecated alias of GraphPyCostModel; GraphPyCostModel is the single integration entry point.
